...
with col_left:
    ...
    # 탭으로 개별 등록 vs 대량 등록 분리
    tab1, tab2 = st.tabs(["➕ 개별 등록", "📥 대량 텍스트로 등록(추천)"])
    ...
    with tab2:
        st.markdown("구글 스프레드시트의 내용을 복사해서 아래에 붙여넣은 뒤 등록 버튼을 누르세요. (형식: 이름, 주소)")
        bulk_input = st.text_area("대량 데이터 입력", placeholder="강민*, 경기 의정부시 배꽃길 63...\n강아*, 경상북도 칠곡군 왜관읍...", height=150, key="input_bulk")
        
        if st.button("🚀 대량 한 번에 등록", use_container_width=True, key="btn_bulk_add"):
            if bulk_input.strip():
                lines = bulk_input.strip().split("\n")
                count = 0
                for line in lines:
                    parts = line.split(",", 1) # 쉼표 기준 첫 번째는 이름, 두 번째는 주소
                    if len(parts) == 2:
                        ...
                        if item not in st.session_state.master_addresses:
                            st.session_state.master_addresses.append(item)
                            count += 1
                st.success(f"총 {count}건이 성공적으로 등록되었습니다!")
                st.rerun()
            else:
